...
const eventos = [
  ...
  {
    nome: "Palestra de Design de Interiores",
    data: "15/06/2025",
    hora: "16h",
    gratuito: false,
    certificado: true,
    imagem: "images/cursos/design.jpg",
    descricao:
      "Aprenda técnicas modernas de design com profissionais renomados.",
    tipo: "Palestra",
  },
  {
    nome: "Simpósio de Engenharia Sustentável",
    data: "20/06/2025",
    hora: "10h",
    gratuito: true,
    certificado: false,
    imagem: "images/cursos/engenharia.jpg",
    descricao: "Debates e palestras sobre práticas sustentáveis na engenharia.",
    tipo: "Simpósio",
  },
];

const universidadesPromotoras = [
  "FAMETRO",
  "UNINORTE",
  "ESBAM",
  "FAMETRO",
...
const descricoesDetalhadas = [
  ...
  "A palestra de Design de Interiores traz especialistas para compartilhar tendências, dicas práticas e cases de sucesso. Ideal para estudantes e profissionais que desejam inovar em projetos residenciais e comerciais.",
  "O Simpósio de Engenharia Sustentável reúne engenheiros, professores e alunos para discutir soluções ecológicas, energias renováveis e responsabilidade ambiental em obras e projetos.",
];
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function filtrarEventos() {
    ...
    return eventos.filter((evento, idx) => {
      ...
      if (universidade) {
        ok = ok && universidadesPromotoras[idx] === universidade;
      }
      if (tipo) {
        ok =
          ok &&
          evento.tipo &&
          evento.tipo.toLowerCase().startsWith(tipo.toLowerCase());
      }
      return ok;
    });
  }
  // Expondo para uso global caso necessário
    ...
    card.innerHTML = `
            <div class="card evento-card h-100 shadow-lg border-0 rounded-4 overflow-hidden animate__animated animate__fadeInUp" style="background: linear-gradient(135deg, #f8fafc 60%, #e0e7ef 100%);">
              <div class="position-relative">
                <img src="${
                  evento.imagem
                }" class="evento-img card-img-top object-fit-cover" alt="Imagem do evento" style="height: 220px; object-fit: cover; border-top-left-radius: 1.5rem; border-top-right-radius: 1.5rem;">
              ...
              <div class="card-body d-flex flex-column">
                <h5 class="card-title fw-bold mb-2 text-primary-emphasis">${
                  evento.nome
                }</h5>
                ...
                <div class="mb-2">
                    <span class="badge bg-white text-primary border border-2 border-primary" style="font-weight: 600; background: #f1f5fa !important;">
                      Universidade: ${universidadesPromotoras[index]}
                    </span>
                </div>
  ...
  function adicionarListeners() {
    document.querySelectorAll(".ver-mais-btn").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        const eventoOriginalIndex = eventos.findIndex(
          (ev) => ev.nome === evento.nome && ev.data === evento.data
        );
        ...
        document.getElementById("eventoModalDescricao").textContent =
          evento.descricao;
        document.getElementById("eventoModalDescricaoDetalhada").textContent =
          descricoesDetalhadas[eventoOriginalIndex] || "";
        document.getElementById(
          "eventoModalUniversidade"
        ).textContent = `Universidade: ${
          universidadesPromotoras[eventoOriginalIndex] || ""
        }`;
...
document.getElementById("filtroTipo").addEventListener("change", () => {
  ...
  document
    ...
    .addEventListener("click", function () {
      const form = document.getElementById("formNovoEvento");
        const priew = document.getElementById("eviewImagem");
      form.reset();
      preview.src = "";
  ...
  document
    .getElementById("formNovoEvento")
    .addEventListener("submit", function (e) {
      e.preventDefault();
      const nome = document.getElementById("novoNome").value.trim();
      const descricao = document.getElementById("novoDescricao").value.trim();
      const descricaoDetalhada = document
        .getElementById("novoDescricaoDetalhada")
        .value.trim();
      ...
      const tipoValorRadio = document.querySelector(
        'input[name="novoTipoValor"]:checked'
      );
      if (!tipoValorRadio) {
      ...
      universidadesPromotoras.unshift(universidade);
      descricoesDetalhadas.unshift(descricaoDetalhada);
      const modal = bootstrap.Modal.getInstance(
        document.getElementById("modalNovoEvento")
      );
      if (modal) modal.hide();
      this.reset();
      previewImagem.src = "";
      ...
      popup.className =
        "alert alert-success position-fixed top-0 start-50 translate-middle-x mt-4 shadow-lg rounded-4 border-0 d-flex align-items-center justify-content-center animate__animated animate__fadeInDown";
      ...
      popup.style.background =
        "linear-gradient(90deg, #22c55e 60%, #16a34a 100%)"; // verde
